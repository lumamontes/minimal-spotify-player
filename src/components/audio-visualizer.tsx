"use client";
import { selectedPresetAtom, trackDataAtom } from "@/utils/atoms";
import { useAtom } from "jotai";
import { fetchAudioAnalysis } from "@/lib/spotify";
import { getMetronome, getWaveform} from '@/utils/audio-processor';
import analysisData  from '../../data.json';
import { useEffect, useRef } from "react";
import * as THREE from "three";
import { gradients, presets } from "@/constants/presets";

export function AudioVisualizer({ accessToken }: { accessToken: string }) {
  // console.log(data);
  const [selectedPreset] = useAtom(selectedPresetAtom);
  const [trackData] = useAtom(trackDataAtom);
  ...
  return (
    <div
      className={`w-full h-full ${selectedPreset} flex justify-center items-center`}
    >
      TrackId: {trackData.id}
      Is paused: {trackData.isPaused ? "true" : "false"}
      ...
      {/* <button onClick={handleAudioAnalysis}>Get Audio Analysis</button> */}
    </div>
  );
}
